...
>
> 1. **Two hosts, two domains** — Vercel for the frontend, Render for the API. That single fact forces everything else: CORS, `SameSite=None; Secure`, and `withCredentials`.
> 2. **Render's free disk is ephemeral**, so the SQLite file dies on every deploy and every wake. Idempotent startup seeding is what keeps the demo populated.
> 3. **`SECRET_KEY` is generated by Render** (`generateValue: true`) — it never exists in the repo or on anyone's screen.
> 4. **226 pytest tests**, all against a throwaway SQLite file injected through `dependency_overrides[get_db]`. The schema tests use AWS's own published examples as fixtures.
> 5. **Zero frontend tests.** Say it before they find it — the UI was verified by driving a browser, which proves it worked once and catches nothing afterwards.
>
> **Read all of it (~14 min).** Part 1 is deployment, Part 2 is testing; both are short.
